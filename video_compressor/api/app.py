...
def create_app():
    ...
    def _cleanup_tasks():
        """Remove tasks that are older than 1 hour and completed."""
        import time
        now = time.time()
        with tasks_lock:
            to_delete = []
            for tid, t in tasks.items():
                # If task is finished (success, failed, cancelled) and old
                if t["status"] in ["success", "failed", "cancelled"]:
                    # We should probably track finish time, but for now just count
                    pass
            # Simplified cleanup: keep only last 100 tasks
            if len(tasks) > 100:
                # Sort by some criteria or just pick oldest? 
                # We don't have timestamps yet. Let's add them.
                pass
